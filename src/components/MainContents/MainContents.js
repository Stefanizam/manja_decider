...
export const MainContents = (props) => {
  ...
  const showEditManjaModal = useSelector(
    (theState) => theState.showEditManjaModal
  );
  ...
  useEffect(() => {
    setDidMount(false);
    if (didMount) {
      fetch("http://localhost:5000/api/v1/manjas", { method: "GET", })
        .then(response => response.json())
        .then(data => setAllManjas(data.data))
    }
  }, [didMount]);
  ...
  const editManjaHandler = () => {
    dispatch({
      type: "SHOW_MANJA_MODAL",
      payload: { show: true, type: "edit" },
    });
  };
  ...
  const updateAllManjas = (theManja, type) => {
    ...
    dispatch({
      type: "SET_SELECTED_MANJA",
      payload: { name: "", id: "", description: "", newDate: "", imageUrl: "" },
    });
    setSortManjas(true);
    if (type === "delete") {
  ...
  return (
    <React.Fragment>
      {showEditManjaModal.show === true && (
        <EditManjaModal
          submitManja={submitManjaHandler}
          hideModal={hideAddNewManjaModal}
          modalType={showEditManjaModal.type}
          updateExistingManja={updateAllManjas}
        />
      )}
      {showDeleteModal && (
        <AreYouSureModal
          manjaName={selectedManja.name}
          hideModal={() => setShowDeleteModal(false)}
          confirmDelete={updateAllManjas}
        />
      )}
      <div ref={whole} className="d-flex justify-content-center mt-5">
          ...
          {allManjas.map((item) => {
            return (
              <Manja
                theManja={item}
                key={item.id}
                updateManjas={updateAllManjas}
                editManja={editManjaHandler}
                deleteManja={() => setShowDeleteModal(true)}
              />
            );
          })}
        </div>
